...
steals = [Steal(), Steal(), Steal(), Steal(), Steal(), Steal()]
...
def punishment(culprit, victim, enforcer):
    ...
    if tokens_allocated and enforcer != culprit:
        ...
        P_victim = 0
        P_innocent = (1 / n_punishable) * (1 - (tokens_allocated / 10))
        P_culprit = (1 / n_punishable)  + ((n_punishable - 1)/n_punishable)* (tokens_allocated / 10)
        P_enforcer = 0
...
def threaded_client(conn, player_no):
    ...
    while True:
        ready[player_no] = pickle.loads(conn.recv(3 * 4096))
        if sum(ready) >= 2:
            victims = [0,0,0,0,0,0]
            # for S in all_steals:
            #     if S[2]:
            ...
            if data_type == "trade_class":
                ...
                if not trade:
                    ...
                    if player.resources[trade.TryToTrade[1]] >= trade.TryToTrade[2]:
                        for p in players:
                            print(p.connected)
                        if players[trade.TryToTrade[0]-1].connected:
                            ...
                            response_message = "Recipient not connected."
                    else:
            ...
            if data_type == "steal_class":
                steal, player, defence_dragging, cross_dragging = pickle.loads(conn.recv(100*4096))
                ...
                if not steal:
                            ...
                            if steal.stealoclock[i]:
                                ...
                                if players[player.stealing_from[i]-1].connected:
                                    ...
                                    if players[player.stealing_from[i]-1].resources["Grain"]:
                                        ...
                                        players[player.stealing_from[i]-1].resources["Grain"] -= 1
                                    else:
                                        pass
                                    ...
                                    steal.initialize_steal_token(i)
                        ...
                        for j in all_steals.keys():  # see all players steal tokens
                            if j != player.id:  # except the players own
                                for stoken_no, stoken in enumerate(all_steals[
                                                                       j]):  # which steal token = stoken_no  and  stoken is the steal token itself
                                    for defence in all_defences[player.id]:  # get players own
                                        if defence.collidepoint((stoken[0], stoken[1])):
                                            print("player ", j, "was punished for stealing from", stoken[2])
                    ...
                    if steal.detectoclockS:
                        ...
                        for i in all_defences.keys():  # all_defences has keys and values as player.id:rects made from deence coordinates
                            if i != player.id:  # we dont want to check the player's own defences matching with his steal tokens
                                for defence in all_defences[i]:  # check all the defence rects in one of the values
                                    for coords_no, coords in enumerate(all_steals[
                                        ...
                                        if defence.collidepoint((coords[0], coords[
                                            ...
                                            print("Player ", player.id, "was punished for stealing from ", coords[2])
                                            ...
                                            steal.initialize_steal_token(coords_no)
                    ...
                    conn.sendall(pickle.dumps((steal, player, police_log)))
        else:
            continue
            # Write data to file
            # if len(new_chat_row):
